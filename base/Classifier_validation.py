...
nltk.download('punkt')
import pandas as pd
df=pd.read_csv('./../data/classifier_data_0.csv',encoding = "latin-1")
...
optimizer = torch.optim.AdamW([
    ...
    {'params': model.classifier.parameters(), 'lr': 1e-5, 'eps':1e-8}
])
...
'''
models_save_dir = './model_save/base_model'
...
for sent in sentences:
    ...
    #   (3) Append the `[SEP]` token to the end.
    #   (4) Map tokens to their IDs.
    encoded_sent = tokenizer.encode(sent, add_special_tokens=True, max_length =128,truncation=True)
    # sent,                      # Sentence to encode.
    # add_special_tokens = True, # Add '[CLS]' and '[SEP]'
    # max_length = 512,
    # truncation=True
