...
'''
classes = df.labels.unique()#np.array(list(set(train_labels)))
print(len(classes))
...
base_classes=213
classes=10
for inc_iteration in range(0, 3):
    if inc_iteration==0:
        sp=0.70
        config = BertConfig(hidden_size=768,
                            ...
                            num_labels=base_classes)
        # new_sd=model.state_dict()
        # Create our custom BERTClassifier model object
        model = BertClassifier(config)
        model = nn.DataParallel(model)
        # model.classifier = nn.Linear(model.classifier.in_features , len(classes)+base_classes,bias=True)
        # model_load_path='./model/pytorch_model.bin'
        # state = torch.load(model_load_path)
        # model.load_state_dict(state['state_dict'])
        # print(model.classifier)
        # model.classifier = nn.Linear(model.bert.in_features , len(classes)+base_classes,bias=True)
        # model= BertModel.from_pretrained('bert-base-uncased',config=config)
        state = torch.load('./../base_model/base_model.pt')
        model.module.load_state_dict(state['state_dict'], strict=False)
        model.module.classifier = nn.Linear(model.module.classifier.in_features, base_classes + classes)
        model.to(DEVICE)
        print(model)
    else:
        sp = sp+0.1
        config = BertConfig(hidden_size=768,
                            ...
                            num_attention_heads=12,
                            intermediate_size=3072,
                            num_labels=base_classes+classes)
        # new_sd=model.state_dict()
        # Create our custom BERTClassifier model object
        model = BertClassifier(config)
        model = nn.DataParallel(model)
        ...
        # model.classifier = nn.Linear(model.bert.in_features , len(classes)+base_classes,bias=True)
        # model= BertModel.from_pretrained('bert-base-uncased',config=config)
        state = torch.load('./../model/new_model_inc_1.pt')
        model.module.load_state_dict(state['state_dict'], strict=False)
        #model.classifier = nn.Linear(model.classifier.in_features, base_classes + classes)
        model.to(DEVICE)
        print(model)


    exemplar_data, test_data, new_data = exemplar.exemplar('./../data/base_train.csv', './../data/i_pl.csv', sp)
    base=pd.read_csv('./../data/base_train.csv')
    base_labels=base['assigned_to']

    new = pd.read_csv('./../data/i_pl.csv')
    new_labels = new['assigned_to']

    data=exemplar.data_preprocess(exemplar_data)
    sentences = data.description.values
    labels = data.assigned_to.values

    tot_labels=base_labels.append(new_labels)
    ...
    main_labels = tot_labels.copy()#train_labels.copy()#tot_labels.copy()#
    ...
    le = LabelEncoder()
    #main_labels = le.fit_transform(tot_labels)  # convert email into integer form
    y_train = le.fit_transform(train_labels)
    y_val = le.transform(validation_labels)  # convert email into integer form
    '''
    from tol_label import TolerantLabelEncoder
    ...
    train_data = TensorDataset(train_inputs,train_segment_ids, train_masks, train_labels)
    train_sampler = RandomSampler(train_data)
    train_loader = DataLoader(train_data, sampler=train_sampler, batch_size=batch_size)

    # Create the DataLoader for our validation set.
    validation_data = TensorDataset(validation_inputs,validation_segment_ids, validation_masks, validation_labels)
    validation_sampler = SequentialSampler(validation_data)
    val_loader = DataLoader(validation_data, sampler=validation_sampler, batch_size=batch_size)



    #classes = new_data.assigned_to.unique()#np.array(list(set(train_labels)))
    #base_classes =159
    #print(len(classes))
    ...
    optimizer = torch.optim.AdamW([
        {'params': model.module.bert.parameters(), 'lr': 5e-5},
        {'params': model.module.classifier.parameters(), 'lr': 1e-5, 'eps':1e-8}
    ])
    ...
    df = pd.read_csv("./../data/inc_test_data.csv")
    print(df)
    ...
    pred = np.argmax(np.array(prediction))
    #pred[2]
    ...
    models_save_dir = './../model/new_model_inc_1'
